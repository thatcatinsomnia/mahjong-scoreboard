<template>
  <div v-if="!isSubmit">
    <Title>編輯設定檔</Title>
    <div class="mb-1 mx-auto flex justify-end max-w-3xl">
      <button class="px-6 py-1 bg-red-500 hover:bg-red-600 transition text-white rounded" @click="deletePref">刪除</button>
    </div>
    <div class="mx-auto px-6 py-10 mb-12 max-w-3xl rounded shadow-lg border-t-4 border-green-500 text-gray-600 ">
      ...
      <div class="mt-4 flex flex-col">
        <button class="py-2  text-white text-lg rounded transition-colors duration-300 bg-yellow-500 hover:bg-yellow-600" 
          @click="onSubmit"
        >
          更新
        </button>
  ...
  <div class="py-20 text-center" v-else>{{ message.error ?? message.default }}</div>
</template>
...
export default {
  ...
  setup() {
    ...
    const id = hashids.decode(route.params.id);
    ...
    const onSubmit = async () => {
      isSubmit.value = true;
      message.default = 'UPDATING...';
      message.error = '';

      const res = await PrefServices.patchById(id, pref);
    ...
    const initEditPage = async () => {
      const res = await Promise.all([
        PrefServices.fetchById(id),
        HandServices.fetchAll()
      ]);

      const { name, rate, price, times, rounds, handPoints } = {...res[0].data};
